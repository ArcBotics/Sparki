...
const uint64_t tx_pipe = 0xDEDEDEDEE7LL;
const uint64_t rx_pipe = 0xDEDEDEDEE9LL;

/****************************************************************************/

void Radio::csn(int mode)
{
...
void Radio::print_observe_tx(uint8_t value)
{
...
Radio::Radio():
  ce_pin(NRF_CE), csn_pin(NRF_CSN), wide_band(true), p_variant(false), 
...
void Radio::setPayloadSize(uint8_t size)
{
...
uint8_t Radio::getPayloadSize(void)
{
  return payload_size;
}
...
void Radio::begin(void)
{
  ...
  openReadingPipe(1,tx_pipe);
  openWritingPipe(rx_pipe);
  startListening();
}
...
void Radio::toggle_features(void)
{
  csn(LOW);
...
void Radio::enableAckPayload(void)
{
  //
...
void Radio::writeAckPayload(uint8_t pipe, const void* buf, uint8_t len)
{
  const uint8_t* current = reinterpret_cast<const uint8_t*>(buf);
...
bool Radio::isAckPayloadAvailable(void)
{
...
void Radio::writeChar(char tx_data){
  openReadingPipe(1,tx_pipe);
  openWritingPipe(rx_pipe);

  stopListening();
  write( &tx_data, sizeof(tx_data) );
  startListening();

  openReadingPipe(1,rx_pipe);
  openWritingPipe(tx_pipe);
}

void Radio::writeInt(int tx_data){
  openReadingPipe(1,tx_pipe);
  openWritingPipe(rx_pipe);
...
void Radio::writeFloat(float tx_data){
  openReadingPipe(1,tx_pipe);
  openWritingPipe(rx_pipe);
...
char Radio::readChar(){
  ...
  openWritingPipe(tx_pipe); 
    ...
    while (!done)
    {
      done = read( &rx_data, sizeof(rx_data) );
    }
    return rx_data;
}
